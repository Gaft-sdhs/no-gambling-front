...
const Ladder = ({ result, startSide, setAnimationFinished, gameStarted }) => {
  const ladderRef = useRef(null);
  const playerRef = useRef(null);
  const [position, setPosition] = useState({ top: '0%', left: startSide === 'left' ? '0%' : 'calc(100% - 40px)' });
    ...
    if (result) {
      ...
      const movePlayer = () => {
        ...
        ladderPositions.forEach((pos, index) => {
          ...
          intervals.push({ top: `${pos}%`, left: currentPosition % 2 === 0 ? '0%' : 'calc(100% - 40px)' });
          newPath.push({ top: `${pos}%`, left: currentPosition % 2 === 0 ? '0%' : 'calc(100% - 40px)', type: 'vertical' });
        ...
        intervals.push({ top: '100%', left: currentPosition % 2 === 0 ? '0%' : 'calc(100% - 40px)' });
        newPath.push({ top: '100%', left: currentPosition % 2 === 0 ? '0%' : 'calc(100% - 40px)', type: 'vertical' });
      ...
      setPosition({ top: '0%', left: startSide === 'left' ? '0%' : 'calc(100% - 40px)' });
  ...
  return (
    <div className="ladder-container" ref={ladderRef}>
      ...
      {gameStarted && ladderPositions.map((pos, index) => (
